...
controlProfile_t *currentControlProfile;
...
uint8_t getCurrentControlProfileIndex(void)
{
	return systemConfig()->controlProfileIndex;
}
...
static void setControlProfile(uint8_t controlProfileIndex)
{
	if (controlProfileIndex < MAX_PROFILE_COUNT) {
		systemConfigMutable()->controlProfileIndex = controlProfileIndex;
		currentControlProfile = controlProfilesMutable(controlProfileIndex);
	}
}
...
static void validateAndFixConfig(void)
{
	...
	if (systemConfig()->controlProfileIndex >= MAX_PROFILE_COUNT) {
		systemConfigMutable()->controlProfileIndex = 0;
	}
	setControlProfile(systemConfig()->controlProfileIndex);
...
void changeControlProfile(uint8_t controlProfileIndex)
{
	if (controlProfileIndex >= MAX_PROFILE_COUNT) {
		controlProfileIndex = MAX_PROFILE_COUNT - 1;
	}
	systemConfigMutable()->controlProfileIndex = controlProfileIndex;
	currentControlProfile = controlProfilesMutable(controlProfileIndex);
	beeperConfirmationBeeps(controlProfileIndex + 1);
}
#endif
